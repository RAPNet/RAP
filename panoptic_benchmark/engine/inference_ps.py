...
from .combine_methods import combine_to_panoptic_heuristic, combine_to_panoptic_RAP
...
def inference_ps(
    ...
    elif cfg.MODEL.SEMANTIC.COMBINE_METHOD == "RAP":
        save_dir_symbol = "_RAP"
    ...
    for images, targets, image_ids in tqdm(data_loader):
        ...
        for image_id, prediction in zip(image_ids, output):
            ...
            if prediction.has_field("relation_val"):
                relation_vals = prediction.get_field("relation_val").tolist()
            else:
            ...
            inst_results = []
            for box, score, relation_val, label, mask in zip(boxes, scores, relation_vals, labels, masks):
                inst_results.append({
                    "box": box,
                    "score": score,
            ...
            if cfg.MODEL.SEMANTIC.COMBINE_METHOD == "heuristic":
                ...
                annotation = combine_to_panoptic_RAP(cfg, img_info, inst_results, sem_results,
                                                           id_generator, segmentations_folder)
