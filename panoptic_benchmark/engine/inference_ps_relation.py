...
from .combine_methods import combine_to_panoptic_for_acc
...
def inference_relation(
    ...
    for images, targets, image_ids in data_loader:
        ...
        for image_id, output, target in zip(image_ids, outputs, targets):
            ...
            inst_results = []
            for box, score, instance_id, relation_val, label, mask in zip(boxes, scores, pred_instance_ids, relation_vals, labels, masks):
                inst_results.append({
                    "box": box,
                    "score": score,
            ...
            # segmentation fusion
            acc_num, score_acc_num, infer_count, count = combine_to_panoptic_for_acc(cfg, img_info, inst_results, 
                                                                                     id_generator, target_relations)
            all_acc_num += acc_num
