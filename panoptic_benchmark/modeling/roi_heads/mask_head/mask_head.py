        ...
        if self.cfg.MODEL.MASKIOU_ON:
            ...
            if not self.training:
                ...
                for i in range(1, len(result)):
                    all_mask = torch.cat((all_mask, result[i].get_field("mask")), 0)
                    all_label = torch.cat((all_label, result[i].get_field("labels")), 0)
                return x, result, {}, roi_feature, all_mask, all_label

            loss_mask, selected_mask, labels = self.loss_evaluator(proposals, mask_logits, targets)
            return x, all_proposals, dict(loss_mask=loss_mask), roi_feature, selected_mask, labels
