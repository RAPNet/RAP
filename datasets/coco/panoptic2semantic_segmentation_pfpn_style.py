...
panoptic_file = "annotations/panoptic_train2017.json"
panoptic_image_folder = "annotations/panoptic_train2017/"
output_folder = "annotations/semantic_train2017/"
...
if __name__ == "__main__":
    ...
    count = 1
    for l in categories:
        category_id, isthing = l["id"], l["isthing"]
        if isthing:
            category_id_to_contiguous_id[category_id] = 54
        else:
            category_id_to_contiguous_id[category_id] = count
            count += 1

    threads = []

    thread_num = 32
    each_num = 1822      # thread_num * each_num >= train image number
